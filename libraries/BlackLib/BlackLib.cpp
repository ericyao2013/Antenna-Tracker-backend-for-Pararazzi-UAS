...
{
	std::string file 		= this->getSlotsFilePath();
	this->dtboFileName 		= "my_" + this->pwmName;
...
bool BlackPWM::setDutyValue(uint32_t DutyVal)
{

	this->pwmErrors->outOfRange = false;
		
		uint32_t period 			= this->getNumericPeriodValue();
		uint32_t writeThis 			= period - DutyVal;
	...
	/*
	if( DutyVal > 20000000 or DutyVal < 1000000 )
	{
		this->pwmErrors->outOfRange = true;
		return false;
	}
